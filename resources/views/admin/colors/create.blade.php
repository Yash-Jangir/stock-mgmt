<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            <a href="{{ route('admin.masters') }}">{{ __('Masters') }}</a> > <a href="{{ route('admin.colors.index') }}">{{ __('Color') }}</a> > {{ __('Create') }}
        </h2>
    </x-slot>
            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-xl">
                    ...
                    <section>
                        ...
                        <form method="post" action="{{ route('admin.colors.store') }}" class="mt-6 space-y-6">
                            @csrf

                            <div>
                                <x-input-label for="code" :value="__('Color Code')" />
                                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full" :value="old('code')" placeholder="{{ __('Color Code') }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('code')" />
                            </div>

                            <div>
                                <x-input-label for="name" :value="__('Color Name')" />
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" placeholder="{{ __('Color Name') }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('name')" />
                            </div>

                            <div>
                                <x-input-label for="color_code" :value="__('Color')" />
                                <x-text-input id="color_code" name="color_code" type="color" class="mt-1 block w-full" :value="old('color_code')" placeholder="{{ __('Color') }}" required />
