                            ...
                            <div class="grid grid-cols-2 gap-4 mb-2">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <x-text-input id="address" name="address" :value="old('address', '')" class="mt-1 block grow" required />
                                    </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-2">
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <x-text-input id="contact_no" name="contact_no" :value="old('contact_no')" class="mt-1 block grow" required />
                                    </div>
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <x-text-input id="email" name="email" :value="old('email')" class="mt-1 block grow" />
                                    </div>
                        ...
                        <div class="mt-8">
                            <div class="grid grid-cols-3 gap-4 mb-2">
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <x-text-input id="total_price" type="number" name="total_price" :value="old('total_price')" class="mt-1 block grow" readonly />
                                    </div>
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <x-text-input id="discount" type="number" name="discount" :value="old('discount')" class="mt-1 block grow" />
                                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody id="row-wrapper" class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach (old('product_id', [null]) as $k => $product_id)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                <x-text-input name="product[]" list="allskus" class="mt-1 block w-full" :value="old('product.'.$k)" placeholder="{{ __('Product') }}" required />
                                                <input name="product_id[]" type="hidden" value="{{ $product_id }}"  data-stock="" data-price="" required />
                                            </td>
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                <x-text-input id="unit_price" name="unit_price[]" type="number" class="mt-1 block w-full" :value="old('unit_price.'.$k)" placeholder="{{ __('Unit Price') }}" required />
                                            </td>
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                <x-text-input id="qty" name="qty[]" type="number" class="mt-1 block w-full" :value="old('qty.'.$k)" placeholder="{{ __('QTY') }}" required />
                                            </td>
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                                <x-text-input id="price" name="price[]" type="number" class="mt-1 block w-full" :value="old('price.'.$k)" placeholder="{{ __('Price') }}" readonly />
                                            </td>
                                            <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
    ...
    <datalist id="allskus">
        @foreach ($products as $product)
            @forelse($product->skus as $sku)
                <option 
                    value="{{ $product->name }} - [{{ $sku->color->name }}] - [{{ $sku->size->name }}]" 
                    data-id="p:{{ $product->id }}|s:{{ $sku->id }}" 
            ...
            @empty
                <option 
                    value="{{ $product->name }}" 
                    data-id="p:{{ $product->id }}|p:{{ $product->id }}" 
        ...
        <script>
            $(document).ready(function() {
                ...
                const formSubmition = (form) => {
                    ...
                    $('[name="qty[]"]').each(function () {
                        if (typeOut) {
                            ...
                            const availStock = +$(this).closest('tr').find(`[name="product_id[]"]`).attr('data-stock');
                ...
                const calculateTotal = () => {
                    let tp = $('[name="price[]"]').toArray().reduce((a, b) => strtonum(a) + ($(b).closest('tr').find('[name="product_id[]"]').val() ? strtonum($(b).val()) : 0), 0);
                ...
                const calculatePrice = (row) => {
                    let product_id = row.find('[name="product_id[]"]').val();
                    if (product_id) {
                ...
                function setupAttributes(input, triggerChange = false) {
                    const hInput = input.closest('tr').find('[name="product_id[]"]');
                    const value  = input.val().trim();

                    const list   = $(`#${input.attr('list')}`);
                    const option = list.find(`option[value="${value}"]`);

                    if (!option.length) {
                        input.val('');
                        hInput.val('');
                        hInput.attr('data-price', '');
                        hInput.attr('data-stock', '');
                    } else {
                        input.val(value);
                        hInput.val(option.attr('data-id'))
                        hInput.attr('data-price', option.attr('data-price'));
                        hInput.attr('data-stock', option.attr('data-stock'));
                    }

                    if (triggerChange) {
                        setTimeout(_ => hInput.change());
                    }
                }
                ...
                $('#add-row').click(function () {
                    $('#row-wrapper').append(`
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                            <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                <x-text-input name="product[]" list="allskus" class="mt-1 block w-full" placeholder="{{ __('Product') }}" required />
                                <input name="product_id[]" type="hidden" data-stock="" data-price="" required />
                            </td>
                            <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                ...
                $('body').on('change', '[name="qty[]"]', function () {
                    ...
                    if (typeOut && qty && product) {
                        let stock = row.find(`[name="product_id[]"]`).attr('data-stock');
