    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center justify-end gap-2">
                ...
                <a href="{{ route('admin.sells.create') }}" class="bg-green-400 px-4 py-2 rounded-md text-white">
                    +Add
                </a>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 relative text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('admin.sells.index') }}" method="GET">
                        ...
                        <div class="grid grid-cols-3 gap-2 mb-2">
                            <div>
                                ...
                                <x-text-input id="customer_name" name="customer_name" type="text" class="mt-1 block w-full" :value="request('customer_name')" placeholder="{{ __('Customer Name') }}"/>
                            </div>
                            <div>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <select name="order_by" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value=""></option>
                                    <option value="client_name" @selected(request('order_by') == 'client_name') >{{ __('Customer Name') }}</option>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-100 dark:bg-gray-800">
                            <tr>
                                ...
                                <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __ ('Customer Name') }}</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($sells as $sell)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                    ...
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ date('Y/m/d', strtotime($sell->slip_date)) }}</td>
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {{ $sell->client_name }}
                                    </td>
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">₹{{ number_format($sell->total_price) }}</td>
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $sell->discount }}%</td>
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        <div class="flex items-center justify-end gap-4">
                                            <a href="{{ route('admin.sells.show', $sell->id) }}">
