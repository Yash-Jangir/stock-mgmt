<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            <a href="{{ route('admin.masters') }}">{{ __('Masters') }}</a> > <a href="{{ route('admin.categories.index') }}">{{ __('Category') }}</a> > {{ __('Edit') }}
        </h2>
    </x-slot>
                    ...
                    <section>
                        ...
                        <form method="post" action="{{ route('admin.categories.update', $category->id) }}" class="mt-6 space-y-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full" :value="old('code', $category->code)" placeholder="{{ __('Category Code') }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('code')" />
                            </div>

                            <div>
                                <x-input-label for="name" :value="__('Category Name')" />
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $category->name)" placeholder="{{ __('Category Name') }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('name')" />
                            </div>

                            <div>
                                <x-input-label for="description" :value="__('Description')" />
                                <x-textarea id="description" name="description" class="mt-1 block w-full" :value="old('description')" placeholder="{{ __('Description') }}" >{{ old('description', $category->description) }}</x-textarea>
                                <x-input-error class="mt-2" :messages="$errors->get('description')" />
                            </div>

                            <div>
                                <x-input-label for="rank" :value="__('Rank')" />
                                <x-text-input id="rank" name="rank" type="number" class="mt-1 block w-full" :value="old('rank', $category->rank)" placeholder="{{ __('Rank') }}" required />
                                <x-input-error class="mt-2" :messages="$errors->get('rank')" />
                            </div>

                            <div class="flex items-center gap-4">
                                <input type="checkbox" id="is_active" name="is_active" class="hidden tgl-icon" @checked($category->is_active) data-target="is_active_icon" />
