    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                <a href="{{ route('admin.masters') }}">{{ __('Masters') }}</a> > {{ __('Category') }}
            </h2>
            <div class="flex items-center justify-end gap-2">
                <x-text-input id="search" name="search" type="text" class="mt-1 block w-full" placeholder="Search..." />
                <a href="{{ route('admin.categories.create') }}" class="bg-green-400 px-4 py-2 rounded-md text-white">
                    +Add
                </a>
            </div>
        </div>
                    ...
                    <form action="{{ route('admin.categories.index') }}" method="GET">
                        ...
                        <div class="grid grid-cols-3 gap-2 mb-2">
                            <div>
                                ...
                                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full" :value="request('code')" placeholder="{{ __('Category Code') }}"/>
                            </div>
                            <div>
                                <x-input-label for="name" :value="__('Category Name')" />
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="request('name')" placeholder="{{ __('Category Name') }}"/>
                            </div>
                            <div>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <select name="order_by" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value=""></option>
                                    <option value="code" @selected(request('order_by') == 'code') >{{ __('Category Code') }}</option>
                                    <option value="name" @selected(request('order_by') == 'name') >{{ __('Category Name') }}</option>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-100 dark:bg-gray-800">
                            <tr>
                                ...
                                <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __ ('Category Code') }}</th>
                                <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('Category Name') }}</th>
                                <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('Rank') }}</th>
                                <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('Status') }}</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            @forelse ($categories as $category)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                    ...
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $category->code }}</td>
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $category->name }}</td>
                                    <td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $category->rank }}</td>
                                    ...
                                    <td class="px-2 py-2 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-4">
                                            ...
                                            <form action="{{ route('admin.categories.destroy', $category->id) }}" onsubmit="return confirm('Are you sure?');" method="post">
                                                @method('delete')
                                                @csrf
