        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center justify-end gap-2">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('admin.stocks.scan', ['type' => 'in'])">
                            {{ __('Stock-In') }}
                        </x-dropdown-link>
                        <x-dropdown-link :href="route('admin.stocks.scan', ['type' => 'out'])">
                            {{ __('Stock-Out') }}
                        </x-dropdown-link>
                    </x-slot>
                </x-dropdown>
                ...
                <div class="p-6 relative text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('admin.stocks.index') }}" method="GET">
                        ...
                        <div class="grid grid-cols-3 gap-2 mb-2">
                            <div>
                                <x-input-label for="code" :value="__('Stock Code')" />
                                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full" :value="request('code')" placeholder="{{ __('Stock Code') }}"/>
                            </div>
                            <div>
                                <x-input-label for="name" :value="__('Stock Name')" />
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="request('name')" placeholder="{{ __('Stock Name') }}"/>
                            </div>
                            <div>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <select name="order_by" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value=""></option>
                                    <option value="code" @selected(request('order_by') == 'code') >{{ __('Stock Code') }}</option>
                                    <option value="name" @selected(request('order_by') == 'name') >{{ __('Stock Name') }}</option>
